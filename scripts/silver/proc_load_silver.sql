...
(
select *, row_number() 
over (partition by cst_id order by cst_create_date desc )  as rw 
from bronze.crm_cust_info where cst_id is not null -- Remove null value of cst_id column
)t where rw = 1 -- In order to remove duplicated cst_id;
...
--INSERT INTO silver.erp_px_cat_g1v2 (
	...
	--subcat,
	--maintenance
--)values ('Unkown','Unknown Category','Unknown Subcategory','n/a')
SET @end_time = GETDATE();
PRINT '>> Load Duration: ' + CAST(DATEDIFF(second,@start_time,@end_time) AS NVARCHAR) + ' seconds.';
