...
BEGIN TRY
    ...
    /* ======================================================================
       FACT: fact_sales
       Joins to dim_dates on full_date to resolve the surrogate date_key
       for order/ship/due dates. Must run AFTER dim_customers/dim_products.
    ====================================================================== */
    ...
    ;WITH order_source AS (
        ...
        LEFT JOIN gold.dim_customers dc ON dc.customer_id = m.sls_cust_id
        LEFT JOIN gold.dim_products dp  ON dp.product_number = m.sls_prd_key
        LEFT JOIN gold.dim_dates od     ON od.full_date = m.sls_order_dt
        LEFT JOIN gold.dim_dates sd     ON sd.full_date = m.sls_ship_dt
        LEFT JOIN gold.dim_dates dd     ON dd.full_date = m.sls_due_dt
    )
    ...
    WHEN MATCHED AND HASHBYTES('SHA2_256', CONCAT_WS('|',
            ISNULL(target.customer_key,-1), ISNULL(target.order_date_key,-1),
            ISNULL(target.shipping_date_key,-1), ISNULL(target.due_date_key,-1),
            CAST(target.sales_amount AS VARCHAR), CAST(target.quantity AS VARCHAR), CAST(target.price AS VARCHAR)))
        <> HASHBYTES('SHA2_256', CONCAT_WS('|',
            ISNULL(source.customer_key,-1), ISNULL(source.order_date_key,-1),
            ISNULL(source.shipping_date_key,-1), ISNULL(source.due_date_key,-1),
